...
//	int size;
//protected:
//	mat4 TranslationMatrix, ModelMatrix;
//	vector<vector<int>> moves;
//	char color;
//public:
//	int x, y;
//	GLuint vertexbuffer, uvbuffer, normalbuffer;
//	bool isNotPiece = true;
//	Piece(int x, int y){ this->x = x; this->y = y; }
...
//	}
//	virtual void moveTo(int x, int y, Piece *board[][9])
//	{
//		getMoves(board);
...
//	}
//	virtual void getMoves(Piece* board[][9]){}
//	void tempToString(){ cout << x << ", " << y << " isNotPiece: " << isNotPiece << endl; }
//};
//
...
//private:
//public:
//	King(int x, int y, GLuint vb, GLuint ub, GLuint nb, int s, char c) : Piece(x, y, vb, ub, nb, s, c){}
//	void getMoves(Piece** board)
//	{
//		moves.clear();
//
//	}
...
//private:
//public:
//	Queen(int x, int y, GLuint vb, GLuint ub, GLuint nb, int s, char c) : Piece(x, y, vb, ub, nb, s, c){}
//};
//
...
//private:
//public:
//	Bishop(int x, int y, GLuint vb, GLuint ub, GLuint nb, int s, char c) : Piece(x, y, vb, ub, nb, s, c){}
//};
//
...
//	mat4 RotationMatrix;
//public:
//	Knight(int x, int y, GLuint vb, GLuint ub, GLuint nb, int s, char c) : Piece(x, y, vb, ub, nb, s, c)
//	{
...
//	}
//	void moveTo(int x, int y, Piece* board[][9])
//	{
...
//private:
//public:
//	Rook(int x, int y, GLuint vb, GLuint ub, GLuint nb, int s, char c) : Piece(x, y, vb, ub, nb, s, c){}
//};
//
...
//	bool firstMove = true;
//public:
//	Pawn(int x, int y, GLuint vb, GLuint ub, GLuint nb, int s, char c) : Piece(x, y, vb, ub, nb, s, c){}
//	void moveTo(int x, int y, Piece* board[][9])
//	{
//		Piece::moveTo(x, y, board);
//		firstMove = false; 
...
//		if (color == 'W')
//		{
//			board.get(x, y + 1).tempToString();
//			if (y != 8 && board.get(x, y + 1).isNotPiece) { addMove(moves, { x, y + 1 }); }
//			board.get(x, y + 2).tempToString();
//			if (firstMove && board.get(x, y + 2).isNotPiece){ addMove(moves, { x, y + 2 }); }
//		}
//		else if (color == 'B')
//		{
//			board.get(x, y - 1).tempToString();
//			if (y != 1 && board.get(x, y - 1).isNotPiece) { addMove(moves, { x, y - 1 }); }
//			board.get(x, y - 2).tempToString();
//			if (firstMove && board.get(x, y - 2).isNotPiece){ addMove(moves, { x, y - 2 }); }
//		}
...
//
//		for (int r = 1; r < 9; r++){ for (int c = 1; c < 9; c++){ board[r][c] = &Piece( r, c ); } }
//
//		length *= scaleFactor;
...
//		Piece* p = board[x1][y1];
//		Piece* temp = &Piece(x1, y1);
//		place(temp);
//		Piece* toDelete = board[p[0].x][p[0].y];
//		board[x][y] = p;
//		p[0].moveTo(x, y, board);
//	}
//	Piece* getPiece(int x, int y){
//		return board[x][y];
//	}
//	void place(Piece* p){ board[p[0].x][p[0].y] = p; }
//	void rotate(mat4 RotateMatrix){ ModelMatrix = RotateMatrix * ScalingMatrix; }
